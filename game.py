...
#     # Bắt đầu nhạc nền nếu chưa chạy (nhạc nền bạn nói đang ổn)
#     try:
...
#     # dùng để giới hạn tần suất phát foot (nếu muốn thêm cooldown)
#     last_foot_play = 0
#     foot_cooldown_ms = 120  # không phát foot quá dày (tùy chỉnh)
...
#                 # dừng nhạc nền & sfx khi đóng cửa sổ
#                 try:
...
#                     # dừng nhạc nền & mọi sfx khi bấm reset
#                     try:
#                         pygame.mixer.music.stop()
#                     except Exception:
#                         pass
...
#             # truyền event vào player để xử lý (Player.handle_input_event)
#             if not paused:
...
#         if not paused:
#             # lưu vị trí trước khi update để kiểm tra xem player có thực sự thay đổi ô grid không
#             prev_pos = tuple(player.pos)

#             # update player (bên trong Player sẽ gọi _move() theo các sự kiện bàn phím đã lưu)
#             player.update(maze)
#             hunter.update(player.pos, maze, mode)

#             # nếu vị trí thay đổi -> phát tiếng bước chân
...
#                 # play lose sound on dedicated channel if possible
#                 if lose_sound:
#                     try:
#                         if lose_channel:
...
#                 # stop background music (như bạn đã làm)
#                 try:
#                     pygame.mixer.music.stop()
#                 except Exception:
#                     pass
...
#     # nếu ra khỏi vòng while (ví dụ do QUIT)
#     try:
#         pygame.mixer.music.stop()
#     except Exception:
#         pass
...
def game_loop(mode):
    ...
    # Sửa dòng này để truyền theme vào Hunter
